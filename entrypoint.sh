...
initialize () {
    ...
    for FILE in "/etc/php/7.4/apache2/php.ini" "/etc/php/7.2/apache2/php.ini" "/etc/php/7.0/apache2/php.ini" "/etc/php5/apache2/php.ini" "/etc/php.ini" "/usr/local/etc/php.ini" "/etc/php/8.3/apache2/php.ini"; do
        if [ -f $FILE ]; then
            PHPINI=$FILE
            break
        fi
...
start_zoneminder () {
    ...
    if [ "$RETVAL" = "0" ]; then
        ...
        find / | grep zonemider
        # cat /etc/zm.conf
	su www-data --shell=/bin/sh --command='/usr/local/bin/zmdc.pl check'
